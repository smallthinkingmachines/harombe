...
The year 2025 saw the emergence of a new class of software vulnerability: the autonomous AI agent compromise. From WhatsApp chat history exfiltration in April to the Smithery registry breach in October that exposed 3{,}000+ MCP servers, a clear pattern emerged---the protocols designed for agent-tool communication provide no security enforcement at all.
...
This gap between protocol specification and security enforcement is not merely theoretical. The 2025 MCP breach timeline (\Cref{sec:threat-model}) demonstrates that every major breach exploited this exact gap.
...
\textbf{gVisor} offers a middle ground: a user-space kernel reducing ${\sim}300$ syscalls to ${\sim}83$ (72\% reduction). Jang et al.\ \citep{jang2022gvisor} quantified gVisor as 4.2--7.5$\times$ more secure than standard runtimes. Young et al.\ \citep{young2019gvisor} measured 11.4$\times$ syscall overhead but near-native CPU performance. Google's Kubernetes Agent Sandbox (2025) chose gVisor as its default runtime for AI agent workloads \citep{google2025agentsandbox}.

\textbf{Firecracker} microVMs \citep{agache2020firecracker} provide hardware-enforced isolation via KVM with 125\,ms boot time and ${<}5$\,MiB overhead. However, Weissman et al.\ \citep{weissman2023firecracker} showed limited Spectre/MDS protection, and Xiao et al.\ \citep{xiao2023microvm} demonstrated exploitable operation forwarding.
...
\textbf{A2: Compromised or Malicious Tool (Supply Chain).} The attacker controls an MCP server the agent connects to. Goal: exfiltrate data, execute arbitrary code on host, pivot to other tools. Example: Smithery registry breach (Oct 2025)---path traversal exposed Docker credentials for 3{,}000+ servers \citep{authzed2025timeline}.
...
\textbf{Environment:} Darwin 25.2.0 arm64, Python 3.14.3, scipy 1.17.0, scikit-learn 1.8.0, detect-secrets 1.5.0. \textbf{Statistical methods:} timing CIs via Student's $t$-distribution ($\text{df} = n{-}1$) with IQR-based outlier filtering (factor = 1.5); detection rates via Clopper--Pearson exact binomial 95\% CIs; 50 warmup iterations before each measurement loop; GC disabled during measurement. \textbf{Date:} 2026-02-11.

All benchmarks exercise the real security modules directly (no Docker, no mocking). For cross-run reproducibility, \texttt{benchmarks/run\_multi.py} invokes the suite $N$ times independently and computes cross-run 95\% CIs using the $t$-distribution over per-run means.
...
\begin{table}[htbp]
\centering
\caption{Full pipeline overhead---all security layers combined ($n=200$, 16 outliers removed).}
\label{tab:pipeline}
\small
\begin{tabular}{@{}lr@{}}
\toprule
...
\textbf{Baseline comparison:} bare operations (no security) take 0.000375\,ms; the full pipeline takes 0.023\,ms---an absolute overhead of ${\sim}0.024$\,ms, or \textbf{0.005\% of a typical 500\,ms LLM inference call}.

\subsection{Detection Effectiveness (RQ2)}

\subsubsection{Secret Scanner True Positive Rate}

Corpus: 180 synthetic secrets (20 per type), all conforming to production regex patterns (\Cref{tab:tp-rate}).

\begin{table}[htbp]
\centering
\caption{Secret scanner true positive rate by type ($n=20$ per type, Clopper--Pearson CIs).}
\label{tab:tp-rate}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
\midrule
AWS keys       & 20 & 20 & 100\% & [0.832, 1.000] \\
GitHub tokens  & 20 & 20 & 100\% & [0.832, 1.000] \\
Slack tokens   & 20 & 20 & 100\% & [0.832, 1.000] \\
Stripe keys    & 20 & 20 & 100\% & [0.832, 1.000] \\
JWT tokens     & 20 & 20 & 100\% & [0.832, 1.000] \\
Private keys   & 20 & 20 & 100\% & [0.832, 1.000] \\
Database URLs  & 20 & 20 & 100\% & [0.832, 1.000] \\
Passwords      & 20 & 20 & 100\% & [0.832, 1.000] \\
API keys       & 20 & 20 & 100\% & [0.832, 1.000] \\
\midrule
\textbf{Overall} & \textbf{180} & \textbf{180} & \textbf{100\%} & \textbf{[0.980, 1.000]} \\
\bottomrule
\end{tabular}
\end{table}
...
Corpus: 199 benign samples across 7 categories (\Cref{tab:fp-rate}). The single false positive was a placeholder token (\texttt{ghp\_xxxx\ldots}) matching the GitHub token pattern---arguably a true positive since the pattern is structurally valid.
...
\midrule
Prose       & 40 & 0 & 0.0\% & [0.000, 0.088] \\
Code        & 30 & 0 & 0.0\% & [0.000, 0.116] \\
Logs        & 30 & 0 & 0.0\% & [0.000, 0.116] \\
Config      & 29 & 0 & 0.0\% & [0.000, 0.119] \\
URLs        & 25 & 0 & 0.0\% & [0.000, 0.137] \\
Hashes      & 25 & 0 & 0.0\% & [0.000, 0.137] \\
Edge cases  & 20 & 1 & 5.0\% & [0.001, 0.249] \\
\midrule
\textbf{Overall} & \textbf{199} & \textbf{1} & \textbf{0.50\%} & \textbf{[0.000, 0.028]} \\
\bottomrule
\end{tabular}
\end{table}
...
Both tools evaluated on the same 379-sample corpus (\Cref{tab:comparison}).

\begin{table}[htbp]
\centering
\caption{Comparative detection: Harombe vs.\ detect-secrets 1.5.0 (379 samples).}
...
\midrule
True Positives   & 180 & 180 \\
False Positives  & 9   & 198 \\
True Negatives   & 190 & 1   \\
False Negatives  & 0   & 0   \\
Precision        & \textbf{0.952} & 0.476 \\
Recall           & 1.000 & 1.000 \\
$F_1$ Score      & \textbf{0.976} & 0.645 \\
\bottomrule
\end{tabular}
\end{table}

Harombe achieves a 22$\times$ reduction in false positives while maintaining perfect recall.

\subsubsection{Adversarial Secret Detection}

\begin{table}[htbp]
\centering
\caption{Adversarial secret detection robustness (Clopper--Pearson CIs).}
...
\midrule
Split secrets (multi-line)  & 5 & 5 & 100\% & [0.478, 1.000] \\
Multiline context (YAML/env)& 7 & 7 & 100\% & [0.590, 1.000] \\
Unicode homoglyphs          & 5 & 2 & 40\%  & [0.053, 0.853] \\
Base64-encoded              & 6 & 1 & 17\%  & [0.004, 0.641] \\
\bottomrule
\end{tabular}
\end{table}
...
Each breach scenario was reproduced programmatically (\Cref{tab:breaches}).

\begin{table}[htbp]
\centering
\caption{Breach prevention analysis: 2025 MCP attacks reproduced against Harombe.}
\label{tab:breaches}
\small
\begin{tabular}{@{}clcl@{}}
\toprule
...
6/7 attacks fully blocked at the security module level. T7 demonstrates defense-in-depth: all 5 independent layers activated simultaneously.
...
\begin{itemize}[nosep]
  ...
  \item Secret corpus uses synthetic secrets; real-world FP rates may differ.
  \item \textbf{Container cold-start latency not yet formally measured} (benchmarks bypass Docker). Preliminary informal testing suggests ${\sim}1$--3\,s cold start for Alpine-based containers, reduced to ${<}500$\,ms with pre-warming.
  \item Base64-encoded secrets (17\% detection) and Unicode homoglyphs (40\%) evade regex-based scanning---consistent with industry limitations \citep{basak2023secrets,orca2024base64}.
...
\textbf{Container Escape.} Recent CVEs underscore this risk: CVE-2024-1086 (99.4\% success rate), CVE-2025-23266 (CVSS 9.0), and three runc escapes in November 2025. Lin et al.\ \citep{lin2018container} found 56.82\% of exploits succeed against default Docker. gVisor mitigates this \citep{jang2022gvisor} but introduces compatibility issues. This reinforces defense-in-depth.

\textbf{Adversarial Evasion.} Regex-based scanning is vulnerable to encoding-based evasion. This is consistent with industry limitations \citep{basak2023secrets,orca2024base64,boucher2023trojan}. Defense paths: Unicode normalization (NFKC) and ML-based scanning \citep{basak2025llmsecrets,wiz2025scanning}.
...
\textbf{Single Point of Failure.} The gateway is a single point of failure. Mitigation: minimal process with narrow API surface; future work on replication and hardware-backed attestation.
...
\begin{itemize}[nosep]
  \item \textbf{6/7 real-world MCP breaches from 2025 fully prevented}
  \item \textbf{${\sim}0.025$\,ms full pipeline overhead} (cross-run 95\% CI: [0.022, 0.028])---0.005\% of a 500\,ms LLM inference call
  \item \textbf{100\% secret detection rate} (180/180, Clopper--Pearson CI: [0.980, 1.000]) with 0.50\% FP rate
  \item \textbf{$F_1 = 0.976$} vs.\ 0.645 for detect-secrets (22$\times$ fewer false positives)
  \item \textbf{Sub-microsecond risk classification}
  \item \textbf{2{,}036 ops/sec audit throughput}
  \item Transparent adversarial boundaries: 100\% for split secrets, 17\% for base64, 40\% for homoglyphs
...
\begin{enumerate}[nosep]
  ...
  \item Encoding-aware scanning (base64, Unicode normalization)
